...
///////////////////////////////////////Inner join/////////////////////
 $ci =& get_instance();
 $ci->load->database();
 $sql="SELECT SUM(patient_test_details.PN_flag) AS POSITIVE FROM patient_test_details INNER JOIN test_master ON test_master.test_type_code =patient_test_details.test_id INNER JOIN disease_subcatagory ON disease_subcatagory.disease_sub_id=test_master.disease_sub_category_id WHERE patient_test_details.institution_code='$institution_code' AND patient_test_details.test_date BETWEEN '$start_date1' AND '$end_date1' AND disease_subcatagory.disease_sub_id='$disease_sub_id '";
$query = $ci->db->query($sql);
$row = $query->result_array();
return $row;
...
$sql=" SELECT COUNT(*) AS CASES FROM admission_details  WHERE admission_details.disease_syndrome_code='$disease_syndrome_id'  AND  admission_details.institution_code='$institution_code' AND DATE_FORMAT(admission_details.admission_date_time, '%Y-%m-%d') BETWEEN '$start_date' AND '$end_date'";
 $ci =& get_instance();
 $ci->load->database();
$query = $ci->db->query($sql);
$row = $query->result_array();
